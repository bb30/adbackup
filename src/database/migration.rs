...
impl DatabaseMigrator {
    ...
    pub fn migrate(conn: &Connection, current_version: u32) -> Result<(), Error> {
        if current_version > CURRENT_VERSION {
            return Err(Error::from(MigratorError::UnknownDatabaseVersion {
        ...
        while ver < CURRENT_VERSION {
            match ver {
                ...
                _ => {
                    return Err(Error::from(MigratorError::NoMigrationFunction {
                        version: ver,
                    }))
                }
            };
...
mod tests {
    ...
    fn test_migration_unknown_version() {
        ...
        assert_eq!(
            format!(
                "{}",
                DatabaseMigrator::migrate(&conn, CURRENT_VERSION + 1).unwrap_err()
            ),
            format!("unknown database version: {}", CURRENT_VERSION + 1)
        );
