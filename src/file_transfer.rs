...
impl FileTransfer {
    ...
    pub fn push(device_id: Option<String>, src_path: String, dst_path: String)
                -> Result<(), Error> {
...
mod tests {
    ...
    fn test_simple_pull() {
        if Device::list_devices().unwrap().len() > 0 {
            assert!(FileTransfer::pull(Some("emulator-5554".to_string()),
                                       "/sdcard/la/".to_string()).is_ok())
        }
    }
    ...
    fn test_simple_push() {
        if Device::list_devices().unwrap().len() > 0 {
            assert!(FileTransfer::push(Some("emulator-5554".to_string()),
